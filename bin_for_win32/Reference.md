...
* [外部プロキシの適用を一部のサイトを見る場合のみに限定する](#proxy2)
...
post_confirm_hostsなどでわざわざ範囲を絞らずにすべてのサイトで有効にすればよいのではないかと思われる方もいるだろうが
...
この接続の許可/不許の防衛機構としてMoaiは２段階設けてある.
...
## <a name="proxy2">外部プロキシの適用を一部のサイトを見る場合のみに限定する
-----------------------------------
...
(たとえばお宝画像を手っ取り早く収集したいだけなのに間にproxyを挟むのは無駄であろう)
...
~~~
    acceptable_host:
      ...
      認めてしまうことになる.  「LAN からの接続のみ許可」といった意味を持つ値を提供したいところではあるが、
...
~~~c
  bool initiate( ZnkMyf ftr_send, const char* parent_proxy,
    ...
    これは処理の成功時、またはエラー発生時における報告メッセージとなる.
