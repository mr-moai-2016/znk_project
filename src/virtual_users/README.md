...
-----------------------------------
  ...
  必要なら[HowToCompile.txt][3]を見てもらいソースコードからコンパイルして戴きたい. 
  (Virtual USERSのソースコードは src/virtual_usersにある).


## <a name="pos5">Virtual USERSの使い方
-----------------------------------
...
> filterやpluginを作成して追加しなければならない.
  ...
  ただ、下記に述べる簡易Webサーバとしてこのツールを使う場合で、かつ他のLAN上のマシンからの接続をしたい場合は、このブロックを解除する必要がある.

  【参考】
~~~
    このダイアログは一体何なのか？  
    少し詳しく述べると、このプログラムが動作しているマシン以外からの接続要求の可能性がある
    プログラムにおいて初回起動時に必ず表示されるものである.
    サーバのようなプログラムを過去にインストールしたことがある方なら見たことがあるという方も
    多いのではないだろうか？
    また表示にMoai for Windowsとあるが、これはVirtual USERSが内部で使用しているローカルプロキシエンジンである.
