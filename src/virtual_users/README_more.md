# Moai/Virtual USERS リファレンスマニュアル
-----------------------------------
...
-----------------------------------
Moai/Virtual USERSでは、HTTPにおけるGETにて受信されるHTMLやJavascriptやCSSにおいて
...
ファイルが作成できたら、Moai/Virtual USERSを起動(再起動)し、この状態で普通に「２ちゃんねる」へとアクセスする.
以下はその表示結果である.  
...
-----------------------------------
Moai/Virtual USERSでは、HTTPにおけるPOSTにて送信されるヘッダやPOST変数、クッキーの値において
...
ファイルが作成できたら、Moai/Virtual USERSを起動(再起動)し、この状態で普通に「ふたばちゃんねる」へとアクセスする.
...
> このようにMoai/Virtual USERSでは、POST変数のフィルタリングを比較的簡易に実現できる.
...
-----------------------------------
Moai/Virtual USERSでは、config.myf 内の ignore_hosts において記載されているホストへブラウザが接続しようとした場合
それをブロックすることができる.
...
これはご家庭にある２台目３代目のマシン等から、Moai/Virtual USERSの起動している１台目のマシンへ接続して
Moai/Virtual USERSの提供する機能を利用するといったような用途を想定している.
...
Moai/Virtual USERSでは次の接続仲介先として外部プロキシを設定することができる.
このときは、概念的に次のような接続になる.

あなたのブラウザ環境 => ローカルプロキシMoai/Virtual USERS => 外部プロキシ => 送信先のサイト
...
Moai/Virtual USERSはその他のローカルプロキシと併用することもできる.
例えばPolipoとのコラボも可能であることを確認している.
これによってPolipoとMoai/Virtual USERSの恩恵を同時に享受できる.
概念的に次のような接続になる.

あなたのブラウザ環境 => ローカルプロキシMoai/Virtual USERS => ローカルプロキシPolipo => 送信先のサイト
...
あなたのブラウザ環境 => ローカルプロキシPolipo => ローカルプロキシMoai/Virtual USERS => 送信先のサイト

この場合はPolipo側の設定ファイルを変更し、Moai/Virtual USERS(ポート8124)へ接続するようにしなければならない.
...
これまでに述べたconfig.myf内の変数をWeb上から確認および設定できるというものである.
  ...
  **Filters and Plugins Initiation**  
  ここではプラグインとして登録されているターゲットのInitiate処理を呼び出す.
  これにより、具体的に何が実行されるかはプラグインによるが、その実行結果がすぐ下に表示される.
  例えば、掲示板の投稿用sendフィルタの変数値をランダマイズ化したり仮想化したりなどがこれにより行える.

  **Functional Configuration**  
  ここではMoaiの機能に関する設定を行う.
  parent_proxy のメニューではconfig.myf内のparent_proxy_listの内容が表示され、
...
プラグインでは以下のダイナミックロード可能なグローバル関数を C 言語により実装する.
~~~
  initiate 
...
(一応、Moai/Virtual USERSの将来のバージョンではそのようなジェネレータの追加も考えてはいる).
...
-----------------------------------
  ...
  参考のため、Moai/Virtual USERSのようなツールを使わず、手動で偽装する方法も記載しておいた.
