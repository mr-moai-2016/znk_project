	...
	if( is_authenticated && IS_OK( varp = ZnkVarpAry_find_byName_literal( post_vars, "command", false ) )){
		...
		if( ZnkS_eq( ZnkVar_cstr(varp), "save_link" ) ){
			...
			ZnkStr_addf( EstLM_edit_ui,
					...
					"<span class=MstyOnlyIE8>説明:</span><input class=MstyInputField type=text name=EstLM_comment placeholder=\"説明\" value=\"\" size=50><br>\n" );
			ZnkStr_addf( EstLM_edit_ui,
			...
			if( edit_idx == Znk_NPOS ){
				...
				if( linf ){
					...
					ZnkStr_addf( EstLM_edit_ui,
							...
							"<span class=MstyOnlyIE8>説明:</span><input class=MstyInputField type=text name=EstLM_comment placeholder=\"説明\" value=\"%s\" size=50><br>\n",
							id, id, url, comment );
					ZnkStr_addf( EstLM_edit_ui,
