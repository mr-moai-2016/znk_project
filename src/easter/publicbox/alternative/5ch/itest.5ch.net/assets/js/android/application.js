...
(function (window, undefined) {
  var _5ch = {
    regexp: {
      sssp: {
        BE: /((sssp):\/\/img\.2ch\.net\/(ico|premium)\/[_\-a-z0-9]*\.gif)/,
    ...
    Ad: {
      ...
      watchStatusOfAdNetworkView: function () {
        ...
        this.lazy_load_interval = setInterval(function () {
          ...
          $js_ad_iframe_wraps.each(function (k, div) {
            ...
            iframe.className = 'js-ad_iframe ad_network_content';
        ...
        $js_ad_iframe_wraps.each(function (k, div) {
          ...
          setTimeout(function () {
            ...
            if (div.classList.contains('js-infeed') && _5ch.env.site_type === '5ch') {
              ad.hash = 'infeed';
              ad.body = "<script src=\"//i2ad.jp/customer/cn2lech/2ch7.php?id=8\"></script>";
            }
            var html = "<body style='margin: 0;' data-itestadid='" + ad.hash + "'>" + ad.body + "</body>";
            if (!iframe.contentWindow) return false;
            var iframe_document = iframe.contentWindow.document;
      ...
      toppage: {
        to_html: function (ad, target) {
          ...
          switch (target) {
          case 'ranking':
            if (ad.adType === 'js') return '<div class="js-ad_iframe_wrap js-disable"><img class="lazy-image" data-original="/assets/img/loading.gif"></div>';
            var html = '';
        ...
        to_html: function (ad, index) {
          ...
          if (document.getElementsByClassName('ad').length == 0) {
            if (3 >= _5ch.Subback.infeed_count) {
              ...
              return '<div class="js-ad_iframe_wrap js-disable js-infeed"><img class="lazy-image" data-original="/assets/img/loading.gif"></div>';
            }
          }
          if (ad.adType === 'js') return '<div class="js-ad_iframe_wrap js-disable"><img class="lazy-image" data-original="/assets/img/loading.gif"></div>';
          var html = '';
    ...
    Bbsmenu: {
      ...
      tabAd: function (target) {
        switch (target) {
        case 'ranking':
          if (!!document.getElementById('js-toppage-' + target + '-instand_ad')) return this;
          var $target_tag = $('[data-content="' + target + '"]');
          if ($target_tag.find('iframe').length > 0) break;
          var show_pos = _5ch.env.ad_interval;
          $.when(_5ch.Ad.download({
            url: _5ch.request.board
          })).done(function () {
            $target_tag.find('.popular_threads_row').each(function (k, v) {
              if ((k % show_pos) == (show_pos - 1)) $(v).after('<div class="popular_threads_row">' + _5ch.Ad.toppage.to_html(_5ch.Ad.random(), 'ranking') + '</div>');
            });
            _5ch.Ad.network();
            $target_tag.find('.popular_threads').append('<div id="js-toppage-' + target + '-instand_ad" class="popular_threads_row center"></div>');
            var scripttag = '<script src="//i2ad.jp/customer/gmn/itest.php?id=5"></script>';
            var targetdom = document.getElementById('js-toppage-' + target + '-instand_ad');
            _5ch.dom.addIframe(targetdom, scripttag, 300, 250);
          });
          break;
        case 'news':
          if (!!document.getElementById('js-toppage-' + target + '-instand_ad')) return this;
          var ads = ['<script src="//i2ad.jp/customer/gmn/itest.php?id=5"></script>', '<script src="//i2ad.jp/customer/gmn/itest.php?id=5"></script>', '<script src="//i2ad.jp/customer/gmn/itest.php?id=5"></script>'];
          $('.tabs-content-list.selected').append('<div id="js-toppage-' + target + '-instand_ad" class="popular_threads_row center"></div>');
          var scripttag = '<script src="//i2ad.jp/customer/gmn/itest.php?id=5"></script>';
          var targetdom = document.getElementById('js-toppage-' + target + '-instand_ad');
          _5ch.dom.addIframe(targetdom, scripttag, 300, 250);
          break;
        ...
        $('.' + target + '_ads').each(function (k, v) {
          if (v.children.length > 0) return true;
          if (ads.length === 0) return false;
          var ad = ads.shift();
          var iframe = document.createElement('iframe');
          iframe.className = "ad-height250px";
          v.appendChild(iframe);
          var html = "<body style='margin: 0;'>" + ad + "</body>";
          if (!iframe.contentWindow) return false;
          var iframe_document = iframe.contentWindow.document;
          iframe_document.open();
          iframe_document.write(html);
      ...
      addEvent: {
        choiceArea: function () {
          ...
          $('#js-areas').change(function () {
            ...
            $.when(_5ch.Search.download(params)).done(function (downloaded) {
    ...
    Thread: {
      ...
      showCardView: function (own, responses, title) {
        ...
        var treeHTML = "";
        if (own != null) {
          treeHTML = "<div class='card_content first js-cardview_ad-320x50 center'></div>";
        ...
        responses.forEach(function (v, k) {
          ...
          if (k === 0 || (own == null && k === 1)) treeHTML += "<div class='" + class_names + " js-cardview_ad-320x50 center'></div>";
      ...
      loadSproutAd: function () {
        var scripttag = null;
        if (_5ch.env.site_type === '5ch' && _5ch.env.board_type === '5ch') {
          scripttag = '<script type="text/javascript" src="https://js.sprout-ad.com/t/223/525/a1223525.js"></script>';
        }
        if (itest5ch.env.site_type === 'bbspink') {
          if (itest5ch.env.board_type === '5ch') {
            scripttag = '<script type="text/javascript" src="https://js.sprout-ad.com/t/223/525/a1223525.js"></script>';
          } else {
      ...
      showBottomAd: function () {
        ...
        case '5ch':
          var ad = '<script type="text/javascript" src="https://js.sprout-ad.com/t/223/581/a1223581.js"></script>';
          break;
        case 'bbspink':
      ...
      social: {
        ...
        twitter: function () {
          ...
          var scheme = 'twitter://post?message=' + encodeURIComponent(title + ' ' + url);
          var redirect = 'https://twitter.com/intent/tweet?' + $.param(params);
          _5ch.Thread.social.intent(scheme, redirect);
        },
        line: function () {
          var url = location.href;
          var title = $.trim($('#title').text());
          ...
          var scheme = 'line://msg/text/' + encodeURIComponent(title + ' ' + url);
          var redirect = 'http://line.me/R/msg/text/?' + encodeURIComponent(title + ' ' + url);
          this.intent(scheme, redirect);
        }
      },
      id_counts: {},
      url: {
    ...
    Search: {
      ...
      findPage: function (fn) {
        ...
        $.when(_5ch.Search.download(params)).done(function (downloaded) {
          if (0 >= _5ch.Search.downloaded.data.total_count || !_5ch.Search.downloaded.data) {
            $('#subback').empty().append('<li><p>検索ワードを含むスレッドは見つかりませんでした。</p></li>');
            return false;
          }
          _5ch.Search.render('#subback-tmpl', "#subback", function () {
            _5ch.Ad.network();
          });
          if (fn) fn();
        }).fail(function () {
    ...
    dom: {
      ...
      addIframe: function (target_dom, tag, width, height) {
        var iframe = document.createElement('iframe');
...
page('content', 'index', function () {
  ...
  i2.db.init();
  i2.dom.nolink();
  i2.Bbsmenu.yokumiru().thread_histories(false);
  i2.addEvent.stop_submit(function () {
    $('#search_board').trigger('click');
  });
  addEvent.searchThread().searchBoard().showBoards().closeBoards();
});
