...
## 2020/01/21 Moai Version 2.2.2 リリース.
...
~~~
	...
	* birdman(Moaiアップグレードを行うプログラムの本体)自体を更新するのためのロジックに誤りがあったのを修正しました.
	...
	* Moai Android APK版 において、起動時にシステム構成に誤りがないかどうかを自動的にチェックし、
	  万一誤りがある場合はその旨をログへ出力する機構を導入しました.

	Easter

	* ふたばちゃんねるの各種機能に関する不具合の修正

	  Easterにおいて、ふたばちゃんねるの「手書き」、「レス削除」、「del」の機能が機能しない不具合がありましたが、
	  これが正常に機能するよう修正しました.
...
## 2019/12/08 Moai Version 2.2.1 リリース.
...
~~~
	...
	* レスポンシブデザインの導入により、スマホでのユーザーマニュアルやツールの画面の見やすさ、使いやすさが改良されています.
...
## 2018/12/25 Moai Version 2.1.5 リリース.
...
~~~
    ...
    * Easter HyperPostの画面において、添付画像を実際に送信する前にこれから送信される画像の内容をプレビュー表示で確認できる機能を追加.
      (特に添付URLや添付クリップボード機能の利用時にもこの確認表示を行うことが可能).

    その他

    * 一括コンパイル用スクリプト make_world において、libressl のビルドがサポートされていなかった不備を修正.
    * ドキュメントの不備を修正.
~~~
...
~~~
    ...
    * クリップボードの内容をそのまま添付画像として投稿する機能を試験的に実装.
    * 広告除去アルゴリズムの微調整.

    CustomBoy

    * User-Agent生成アルゴリズムにおいて、ChromeのKnowledge-baseの更新と選択確率の調整変更.
...
~~~
    ...
    * Moaiアップグレード(Birdman)における不具合の修正.
    * HTTPヘッダフィルター(特にUser-Agent)に「UNTACHE」値を指定すると、フィルタ処理を行わないように仕様を修正.
    * config_cgi.myf 内における fsys_map_list セクションにあった不具合を修正.

    Easter

    * easter.myfにEasterがデフォルトとして名乗れるUser-Agentを指定できるようにした.
      HTTPヘッダフィルターに「UNTACHE」値が指定されるなどしてフィルター機能が働かない場合、
      easterはこのデフォルトUser-Agentを送信するようにした.
    
    * 万一クラッシュするなどしてフィルターが壊れた場合、最低限の修復を自動的に行う機能.

    CustomBoy

    * 万一クラッシュするなどしてフィルターが壊れた場合、VirtualUSERS等の実行のタイミングで最低限の修復を自動的に行う機能.
