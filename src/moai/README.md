...
-----------------------------------
  ...
  <ul>
  <li>GET時におけるテキストデータの文字列置換フィルタリング等が容易に行える.
      またpluginを導入することによりさらに高度で自由なフィルタリングを行うこともできる.</li>
  <li>POST時におけるHTTPヘッダ、クッキー、POST変数に関する改変フィルタリングが容易に行える.
      またpluginを導入することによりさらに高度で自由なフィルタリングを行うこともできる.</li>
  <li>掲示板投稿時の際、その投稿内容の確認表示が可能で、その処理を解析するのに最適である.
      またpluginを導入することにより、あなたのマシン環境を仮想化、ランダマイズ化する
      VirtualUSERS Initiation機能を搭載している.</li>
  ...
  <li>HTTP CONNECTによるトンネリングによりHTTPSを仲介することができる.
      例えばGoogle検索やYoutube、nico動画などを見ることも可能である.</li>
  </ul>
