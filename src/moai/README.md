...
-----------------------------------
  ...
  このツールはLinuxでも動作し、Linux用の実行用バイナリも一応同梱してあるが、ディストリ
  ビューションやそのライブラリのバージョン構成によっては、同梱の実行バイナリが起動できない
  ということも有り得る. その場合、HowToCompile.txtを見てもらいソースコードからコンパイル
  したものを使って戴きたい.
  (これのソースコードは src/moaiにある).
...
-----------------------------------
  ...
  Juliusz Chroboczek氏には感謝する. ポート番号8123というのもPolipoで使われていたもの
  であり、それに敬意を表しmoaiでもこれをデフォルトポートとした.
...
-----------------------------------
  ...
  (つまりそれに備えた様々な防衛機構は現在、機能として全く組み込まれていない).
