...
ちなみにここでのパターンの記述においては一行につき一箇所のみにワイルドカードを使うことができる.
これにより記述はさらに簡潔になるだろう.
...
ちなみにpost_confirm_hostsのパターンの記述においては一行につき一箇所のみにワイルドカードを使うことができる.
これにより記述はさらに簡潔になるだろう.
...
また、例えばふたば用のアドオン赤福などを使用している場合、このモードがonの場合に内部処理が競合し、
レス送信などがうまく行えない場合があるので、その場合もやはりこれを無効にしておく必要がある.
...
-----------------------------------
  ブラウザのURL指定欄に http://127.0.0.1:8124/configと指定することで表示させることができる.
  ...
  **Functional Configuration**  
  ...
  parent_proxy のメニューでは parent_proxy.txt の内容が表示される.
...
~~~
  initiate 
	...
	「Virtual USERS Initiation」ボタンを押すとこの関数が呼び出される.
  on_post_before
