...
{
	struct rra_addrinfo hints, *res, *res0;
	...
	if (rra_getaddrinfo(h, p, &hints, &res0) != 0) {
		...
		if (rra_getaddrinfo(h, p, &hints, &res0) != 0) {
			...
			if ((s = rra_getaddrinfo(h, p, &hints, &res0)) != 0) {
...
tls_connect_common(struct tls *ctx, const char *servername)
{
	 ...
	 */
	if (servername != NULL &&
	    rra_inet_pton(AF_INET, servername, &addrbuf) != 1 &&
	    rra_inet_pton(AF_INET6, servername, &addrbuf) != 1) {
