...
-----------------------------------
  ...
  <ul>
  <li>GET時におけるテキストデータの文字列置換フィルタリング等が容易に行える.</li>
  <li>POST時におけるHTTPヘッダ、クッキー、POST変数に関する改変フィルタリングが容易に行える.</li>
  <li>掲示板投稿時の際、その投稿内容の確認表示が可能で、その処理を解析するのに最適である.</li>
  ...
  また処理の透明性、安全性、移植性、および動作環境に関して以下の点が挙げられる.
  ...
  <li>実行バイナリはWindowsやLinuxで動作する(未確認だがMacOSでも多分大した修正なくいけるだろう).</li>
  </ul>
...
-----------------------------------
  ...
  またプラグインを開発する際のヒントなどを[Hacking][2]にまとめておいたので興味があれば
  参照されたい.
